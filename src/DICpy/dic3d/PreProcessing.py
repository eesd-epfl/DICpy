    ...
    def read_speckle_images(self, path=None, extension=None, file_names=None, ref_id=0, verbose=False):
        ...
        **Output/Returns:**
        ...
        if isinstance(path, list):
            if len(path) < 2:
                raise ValueError('DICpy: `path` should have the path for images corresponding to each camera.')
            else:
                self.num_cams = len(path)
        else:
            raise TypeError('DICpy: `path` must be a list of strings.')
        ...
        for i in range(self.num_cams):
            ...
            if file_names is None:
                file_names = [f for f in os.listdir(path_) if f.endswith('.' + extension)]
        ...
        for i in range(self.num_cams):
            if verbose:
                print('DICpy: reading speckle images from camera:  '+str(i))

            file_names = file_names_list[i]
            # todo: check if both path have the same number of images.
            images = []
            images_normalized = []
            for f in file_names:
